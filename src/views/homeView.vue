...
<template>
  <div>
    <div>
      <header class="w-full px-4 lg:px-32 xl:px-32 py-4 flex justify-between items-center lg:border-b xl:border-b "
        :class="store.token ? 'border-b' : ''">
        ...
        <nav class=" w-full lg:flex lg:justify-end xl:flex xl:justify-end hidden lg:block xl:block">
          ...
          <RouterLink to="/views/profile" v-if="userProfile">
            <section class="flex items-center">
              <p> <img :src="`https://hilarious-sfogliatella-c9ed00.netlify.app/src/assets/uploads/${userProfile}`"
        ...
        <button @click="showMenu ? cancelMenu() : toggleMenu()"
          ...
          <svg v-if="!showMenu" xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 mb-1 md:mb-0 lg:mb-0 xl:mb-0"
      ...
      <nav class="toast-container lg:hidden xl:hidden bg-white text-gray-600 fixed h-full w-2/3 top-0 my-auto border-r "
        style="z-index: 99;" v-show="showMenu" :class="{ 'animate-slide-in': showMenu, 'animate-slide-out': slideOut }">
        ...
        <p @click="store.token ? routeToArtist() : nonAuthArtist()"
          class="font-bold text-gray-500 outfit-header cursor-pointer">
    ...
    <chatComponent v-show="showMessage" @toggle-visibility="toggleVisibility"
      :class="{ 'animate-slide-in-chat': showMessage, 'animate-slide-out-chat': closeChat }"
